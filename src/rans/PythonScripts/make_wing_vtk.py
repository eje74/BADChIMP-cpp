...
def find_interpolation_lines_old(interpolation_points):
    """
    Us the output from `find_interpolation_points` to find a set of unique 
    line segments that can be used for second order interpolation.

    Parameters
    ----------
    interpolation_points : list-like 
        Output from `find_interpolation_points`

    Return
    ------
    list-like
        list of line segments on the form
        [(p0, p1), (p2, p3), ...]
        where the pn's are tuples given the position of nodes
    """
    ret = []
    for p0, p1, p2, p3 in interpolation_points: 
        # p0-p3 are the points used for the bi-linear interpolation
        ret.append(tuple(sorted((p0, p1))))
        ret.append(tuple(sorted((p1, p2))))
        ret.append(tuple(sorted((p2, p3))))
        ret.append(tuple(sorted((p3, p0))))
    return list(set(ret))
...
if __name__ == "__main__":
    ...
    for rank in range(1, 2):
        ...
        interpolation_points = find_interpolation_points(ij, geo)
        #interpolation_lines = find_interpolation_lines_old(interpolation_points)

        #data = write_boundary_file(destination_folder + f"boundary{rank-1}.txt", ij, interpolation_points, lin_seg, basis)
        data_node = write_boundary_file_node(
        ...
        for px, py, gamma, p0x, p0y, p1x, p1y, p2x, p2y, p3x, p3y, ip_gamma, *tmp, nx, ny in data_node:
            ...
            y = [py, py - gamma*ny]
            plt.plot(x, y, 'k-')       
            # x = [px, (p0x + p1x + p2x + p3x)/4]       
            # y = [py, (p0y + p1y + p2y + p3y)/4]
            x = [px, px + ip_gamma*nx]       
            y = [py, py + ip_gamma*ny]
            plt.plot(x, y, 'c-')
        ...
        # for px, py, q, gamma, p0x, p0y, p1x, p1y, p2x, p2y, p3x, p3y, w0, w1, w2, w3, *tmp in data:       
        #     c = basis[int(q)]
        #     nx = int(px) - c[0]
        #     ny = int(py) - c[1]
        #     if gamma >= 0:
        #         x = [px - gamma*c[0], (p0x + p1x + p2x + p3x)/4]
        #         y = [py - gamma*c[1], (p0y + p1y + p2y + p3y)/4]
        #         plt.plot(x, y, 'k-')
        #         if geo[nx, ny] > 0:
        #             print("ERROR")
        #     elif geo[nx, ny] == 0:
