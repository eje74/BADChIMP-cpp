...
void halfWayBounceBack(
  LbField<DXQY> &f, 
  const ScalarField &rhos, 
  const LbField<DXQY> &gammas, 
  const LbField<DXQY> &fBcQ, 
  const LbField<DXQY> &fBcQHat, 
  ...
  )
{
  for (int bndNo = 0; bndNo < boundary.size(); ++bndNo)
  {
    ...
    {
      const lbBase_t gamma = gammas(0, q, bndNo);
      const lbBase_t fq = fBcQ(0, q, bndNo);
      const lbBase_t fqhat = fBcQHat(0, q, bndNo);
      const lbBase_t rho = rhos(0, nodeNo);
      const lbBase_t cu = 0;

      f(0, q, nodeNo) = (2*gamma*fq + fqhat + 2*LT::w[q]*LT::c2Inv*rho*cu)/(1+2*gamma);
    }
  }
}
...
int main()
{
  ...
  // vtklb.toAttribute("ii");
  // for (int n = vtklb.beginNodeNo(); n < vtklb.endNodeNo(); ++n)
  // {
  //   int x = vtklb.getScalarAttribute<int>();
  //   auto pos = grid.pos(n);
  //   if ((x-pos[0]) != 0)
  //   {
  //     std::cout << "x = " << x << "  and pos = " << pos[0] << "\n";
  //   }
  // }

  // vtklb.toAttribute("jj");
  // for (int n = vtklb.beginNodeNo(); n < vtklb.endNodeNo(); ++n)
  // {
  //   int y = vtklb.getScalarAttribute<int>();
  //   auto pos = grid.pos(n);
  //   if ((y-pos[1]) != 0)
  //   {
  //     std::cout << "y = " << y << "  and pos = " << pos[1] << "\n";
  //   }
  // }

  // ScalarField geotype(1, grid.size());
  // for (int nodeNo = 1; nodeNo < grid.size(); ++nodeNo)
  // {
  //   geotype(0, nodeNo) = nodes.getType(nodeNo);
  // }
  // ScalarField bndmarker(1, grid.size());
  // auto bndNodes = findFluidBndNodes(nodes);
  ...
  // Output<LT> testout(grid, bulkNodes, outputDir, myRank, nProcs);
  // testout.add_file("geo");
  // testout.add_scalar_variables({"node_type", "boundary"}, {geotype, bndmarker});
  // testout.write();
  ...
  HalfWayBounceBack<LT> fluidSolidBoundary(fluidSolidBoundaryNodes, nodes, grid);
  Boundary<LT> boundary(fluidSolidBoundaryNodes, nodes, grid);
  // Gamma vector                                
  //------------------------------------------------------------------------------------- Gamma vector
  LbField<LT> gamma(1, boundary.size());
  ...
  // fBcQHat                                
  //------------------------------------------------------------------------------------- fBcQHat
  LbField<LT> fBcQ(1, boundary.size());
  LbField<LT> fBcQHat(1, boundary.size());


  std::cout << "Number of boundary nodes = " << fluidSolidBoundaryNodes.size() << "\n";
  std::cout << "Boundary size            = " << boundary.size() << "\n";

  //------------------------------------------------------------------------------------- test bc
  //halfWayBounceBack(f, rho, gamma, fBcQ,fBcQHat, boundary, grid);
  ...
  output.add_vector_variables({"vel", "bodyforce"}, {vel, bodyForce});
  ...
  for (int iteration = 0; iteration <= nIterations; iteration++)
  {
    ...
    for (auto nodeNo : bulkNodes)
    {
      ...
      const std::valarray<lbBase_t> force = bodyForce(0, nodeNo);
      // Macroscopic values
      //--------------------------------------------------------------------------------- Macroscopic values
    ...
    for (int bndNo=0; bndNo < boundary.size(); ++bndNo)
    {
      ...
      for (auto q: boundary.unknown(bndNo))
      {
        const int qHat = LT::reverseDirection(q);
        fBcQ(0, q, bndNo)    = fTmp(0, q, grid.neighbor(q, nodeNo));
        fBcQHat(0, q, bndNo) = f(0, qHat, nodeNo);
      }
    }
    ...
    // Fluid solid
    //----------------------------------------------------------------------------------- Fluid solid
    //fluidSolidBoundary.apply(f, grid);
    halfWayBounceBack(f, rho, gamma, fBcQ,fBcQHat, boundary, grid);  
