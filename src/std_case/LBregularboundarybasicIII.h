#ifndef LBREGULARBOUNDARYBASICIII_H
#define LBREGULARBOUNDARYBASICIII_H
...
class RegularBoundaryBasicIII
{
public:
    RegularBoundaryBasicIII(
 ...
 ********************************/
template <typename DXQY>
RegularBoundaryBasicIII<DXQY>::RegularBoundaryBasicIII(
        const std::vector<int>  & boundaryNodes,
        const ScalarField       & qDist,
        const VectorField<DXQY> & normals,
        const VectorField<DXQY> & tangents,
 ...
 * **************************************/
template <typename DXQY>
void RegularBoundaryBasicIII<DXQY>::apply(
        const int                 fieldNo,
        LbField<DXQY>           & f,
        const ScalarField       & qDist,
        const VectorField<DXQY> & normals,
...
)
{
    ...
    for (int bndNo = 0; bndNo < bnd_.size(); bndNo++) {
        ...
        for ( int alpha = 0; alpha < DXQY::nQ; ++alpha) {
         ...
         std::vector<lbBase_t> df(DXQY::nQ, 0);
         for ( auto alpha: bnd_.knownDirs(bndNo) ) 
         {             
            df[0] += (f(fieldNo, alpha, nodeNo) - fReg[alpha])*DXQY::c(alpha, 0);
            df[1] += (f(fieldNo, alpha, nodeNo) - fReg[alpha])*DXQY::c(alpha, 1);            
         }         
